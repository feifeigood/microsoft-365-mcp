/**
 * Regression test for the fork-specific feature: MCP discovery requests
 * (initialize, tools/list, etc.) are allowed through the HTTP bearer-token
 * middleware WITHOUT a token, so an MCP gateway can enumerate the tool catalog
 * before any user has authenticated. Non-discovery requests (e.g. tools/call)
 * still require a valid bearer token.
 *
 * This guards against the patch being silently lost on future upstream rebases.
 */
...
describe('discovery requests bypass bearer auth', () => {
  ...
  it('trustProxyAuth skips the check entirely', () => {
